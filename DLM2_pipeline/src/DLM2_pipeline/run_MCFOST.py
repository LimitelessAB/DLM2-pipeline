...
def find_MCFOST_options(MCFOST_image_parameters,Rout,PSF_parameters,f=3): #f is how much space should the disk take in the image
    ...
    for i in range (0,len(PSF_parameters.Pxscl_list)):
        if PSF_parameters.Pxscl_list[i]!=None: #PSF list should match MCFOST image list, but if there is no PSF or no desired convolution, then just place None in the PSF list slots
            zoom=0
            reshape=0
            option=0
            C1 = MCFOST_image_parameters.Pxscl_list[i] / PSF_parameters.Pxscl_list[i]
            C2 = f * Rout * 1.496e+11 / (MCFOST_image_parameters.distance * 3.086e+16 * C1)
            C3 = PSF_parameters.Nx_list[i] / (MCFOST_image_parameters.Nx_list[i] * C2)
            ...
            if C3>1: #This case means that the resulting image needs to be enlarged to match PSF number of pixels
                ...
                option=2
            else: #This case means that the resulting image maths the PSF number of pixels, there is no need to change Nx
                zoom = C1 
                reshape = MCFOST_image_parameters.Nx_list[i]
                option=3
            options_list[i,:]=[zoom,int(reshape),option]
        else:
            options_list[i,:]=[None,MCFOST_image_parameters.Nx_list[i],None]
    return(options_list)

'---Convolution util---'
###Code of the following function by Éric Thiébaut###
...
def convolution(file_number,wavelength,options,PSF_path,Image_path):
    ...
    if options[2]==1 or options[2]==3:
        MTF=np.fft(np.ifftshift(PSF)) #If we don't ifftshift our convolution will make for corner images on image corners.
        Convolved=np.ifft(MTF*np.fft(MCFOST_image[0,0,0,:,:]))
            
    else:
        Resized_shifted_psf=np.ifftshift(zeropad(PSF,options[1])) #To preserver ou disk resolution, we have made the image larger than the PSF. We therefore increase it's size.
        MTF=np.fft(Resized_shifted_psf)
        Convolved=np.ifft(MTF*np.fft(MCFOST_image[0,0,0,:,:]))
...
def measure_dneb_and_radius(convolution_triggered,wavelengths,nb_px_list,physical_size,pxscl_list,path,f=3):
    ...
    for wavelength_number in range(0,len(pxscl_list)):
        ...
        print("This is "+str(wavelengths[wavelength_number])+" microns.")
        if convolution_triggered==True:
            img_to_fit = (fits.open(path+str(wavelengths[wavelength_number])+'/convolved.fits')[0].data)
        else:
            img_to_fit = (fits.open(path+str(wavelengths[wavelength_number])+'/RT.fits.gz')[0].data)[0,0,0,:,:]
        ...
        maxoffset_arcsec = (1/f * nb_px_list[wavelength_number] * pxscl_fit)/2 # Maximum distance along vertical direction to find the local maximum [arcsec]
        ...
        sampl = 0.01 # Sampling for finer polynomial evaluation and finding minimum distance [arcsec]
        ...
        ax3.set_ylabel(r'$\Delta$X (arcsec)')
...
def complete_run_MCFOST_function(file_number,DustPy_path,settling_type,para_path,convolution_triggered,Rout,MCFOST_image_parameters,PSF_parameters=None):
    ...
    for wavelength_number in range (0, len(MCFOST_image_parameters.wavelength_list)):
        ...
        if convolution_triggered[wavelength_number]==True:
            ...
            if options_list[wavelength_number,2]==1 or options_list[wavelength_number,2]==2:
                ...
                run(para_path,options="-root_dir data"+str(file_number)+" -density_file "+DustPy_path+"/data"+str(file_number)+"_density_"+str(settling_type)+"_settling_file.fits -img "+str(MCFOST_image_parameters.wavelength_list[wavelength_number])+" -resol "+str(options_list[wavelength_number,1])+" "+str(options_list[wavelength_number,1])+" -zoom "+str(options_list[wavelength_number,0]))
            else:
                run(para_path,options="-root_dir data"+str(file_number)+" -density_file "+DustPy_path+"/data"+str(file_number)+"_density_"+str(settling_type)+"_settling_file.fits -img "+str(MCFOST_image_parameters.wavelength_list[wavelength_number])+" -zoom "+str(options_list[wavelength_number,0]))    
            #Convolve MCFOST image
            Convolved_image=convolution(file_number,MCFOST_image_parameters.wavelength_list[wavelength_number],options_list[wavelength_number,:],PSF_parameters.path+"/PSF-"+str(MCFOST_image_parameters.wavelength_list[wavelength_number])+".fits",DustPy_path+"/data"+str(file_number)+"/data_"+str(MCFOST_image_parameters.wavelength_list[wavelength_number])+'/RT.fits.gz')
    ...
    if convolution_triggered != ([False]*len(MCFOST_image_parameters.wavelength_list)):
        ...
        Dark_lanes, radius_list = measure_dneb_and_radius(convolution_triggered,MCFOST_image_parameters.wavelength_list,options_list[:,1],(1/options_list[:,0])*MCFOST_image_parameters.Grid_size,PSF_parameters.Pxscl_list,DustPy_path+"/data"+str(file_number)+"/data_")
    else:
        ...
        Dark_lanes, radius_list = measure_dneb_and_radius(convolution_triggered,MCFOST_image_parameters.wavelength_list,MCFOST_image_parameters.Nx_list,MCFOST_image_parameters.Grid_size,MCFOST_image_parameters.Pxscl_list,DustPy_path+"/data"+str(file_number)+"/data_")
